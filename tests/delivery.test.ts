...
describe.skipIf(process.env.STUDIO_DATABASE_TESTS !== "true")(
  ...
  () => {
    ...
    beforeAll(async () => {
      ...
      await enqueue("initialization", "whatsapp", payload, sql);
    });
    afterAll(async () => {
    ...
    beforeEach(async () => {
      await sql`TRUNCATE studio_deliveries`;
      sends = 0;
      calls = 0;
      messages = [];
      ids = [];
    ...
    test("durable ingress deduplicates and concurrent workers dispatch once", async () => {
      const inserts = await Promise.all(
        Array.from({ length: 4 }, () =>
          enqueue("duplicate", "whatsapp", payload, sql),
        ),
      );
      ...
      const [row] =
        await sql`SELECT * FROM studio_deliveries WHERE id='duplicate'`;
      ...
      expect(await enqueue("duplicate", "whatsapp", payload, sql)).toBe(false);
    });

    test("timeout after dispatch is uncertain and never automatically sent again", async () => {
      await enqueue("unknown", "whatsapp", payload, sql);
      deps.send = async () => {
        sends++;
    ...
    test("a success response without provider receipt stays uncertain", async () => {
      await enqueue("missing-receipt", "whatsapp", payload, sql);
      deps.send = async () => {
        sends++;
      ...
      expect(sends).toBe(1);
      const [row] =
        await sql`SELECT result FROM studio_deliveries WHERE id='missing-receipt'`;
      expect(row.result.code).toBe("RECEIPT_MISSING");
    });

    test("revocation after inference blocks the final send", async () => {
      await enqueue("revoked", "whatsapp", payload, sql);
    ...
    test("failed inference and unsupported channels never report successful delivery", async () => {
      await enqueue("failed-run", "whatsapp", payload, sql);
      const originalRun = deps.run!;
      deps.run = async (...args) => {
        const result = await originalRun(...args);
      ...
      await sql`INSERT INTO studio_deliveries (id,kind,payload) VALUES ('unsupported','github',${sql.json(payload)})`;
      expect(await drainDeliveries(deps)).toEqual([
    ...
    test("stale pre-send recovery marks interrupted inference and uses a new attempt", async () => {
      await enqueue("stale", "whatsapp", payload, sql);
      ...
      await sql`UPDATE studio_deliveries SET status='running',attempts=1,updated_at=now()-interval '6 minutes' WHERE id='stale'`;
      expect(await drainDeliveries(deps)).toEqual([
      ...
      expect(sends).toBe(1);
      await enqueue("exhausted", "whatsapp", payload, sql);
      await sql`UPDATE studio_deliveries SET status='running',attempts=3,updated_at=now()-interval '6 minutes' WHERE id='exhausted'`;
      expect(await drainDeliveries(deps)).toEqual([]);
      const [row] =
        await sql`SELECT status FROM studio_deliveries WHERE id='exhausted'`;
      expect(row.status).toBe("failed");
      expect(sends).toBe(1);
    });

    test("expired sending is uncertain and loss of lease prevents stale dispatch", async () => {
      await enqueue("expired-send", "whatsapp", payload, sql);
      await sql`UPDATE studio_deliveries SET status='sending',attempts=1,updated_at=now()-interval '6 minutes' WHERE id='expired-send'`;
      expect(await drainDeliveries(deps)).toEqual([]);
      const [expired] =
        await sql`SELECT status FROM studio_deliveries WHERE id='expired-send'`;
      expect(expired.status).toBe("uncertain");
      await enqueue("lease-lost", "whatsapp", payload, sql);
      const originalRun = deps.run!;
      deps.run = async (...args) => {
        const result = await originalRun(...args);
        await sql`UPDATE studio_deliveries SET attempts=2,status='queued' WHERE id='lease-lost'`;
        return result;
      };
      expect(await drainDeliveries(deps)).toEqual([
      ...
      expect(sends).toBe(0);
      const [lost] =
        await sql`SELECT status,attempts FROM studio_deliveries WHERE id='lease-lost'`;
